...
For a starter, let's identify keywords for movies that recorded profit of $ 50 million and above. 75% (3rd Quartile) of movies in the data set recorded profitability of $ 55 million and above, hence I am doing deep dive into keywords of Top 25% of movies.
...
Based on Figure 4, it is clear that genres such as _Comedy_, _Romance_, _Horror_, _Crime_, _Thriller_, _Drama_, _Adventure_ and _Action_ top the list of profit-making movies. Movies that are loss-making / break-even also share the same genres.
...
Figure 6 shows globally well-known production companies, for instance, _Universal Pictures_, _Warner Bros_ and _Twentieth Century_.
...
movie.df %>%
  ...
  correlate() %>%
  autoplot() + labs(
    title = "Figure 9: Correlation among Key Variables"
  )
```

Figure 9 above shows budget, revenue, vote count and popularity are the main factors contribute significantly to profitability.
...
Figure 10 produces an interesting finding where popularity of a movie contribute to profitability only to certain extent. Among all movies, a few movies were extremely popular (popularity > 500) and only one hit $ 1 billion profit. In contrary, there are also less popular movies (popularity < 250) and recorded $ 1 billion and more in profit.
...
Refer to Figure 11, the trend is more obvious where higher vote count translates to higher profit. This is especially true where vote count is within the range of 7,500.
